...
export default function MeltIceGame() {
  ...
  // الحروف معتمدة على الفتحة والكسرة فقط
  const letters = ["بَ", "إِ", "زِ", "شَ", "ثَ", "وِ", "دَ", "قِ", "ذِ", "كَ", "أَ", "سِ", "وَ", "رِ", "دِ"];
  // الهدف الوحيد هو حركة الكسر فقط (الفتحة لن تذوب الجليد وسيعمل صوت الخطأ عند النقر عليها)
  const cutLetters = ["إِ", "زِ", "وِ", "قِ", "ذِ", "سِ", "رِ", "دِ"];
  ...
  const handleTileClick = async (idx) => {
    ...
    if (cutLetters.includes(letters[idx])) {
      ...
      setPoints(newPoints);
      if (newPoints >= cutLetters.length) setWin(true);
      await new Promise(r => setTimeout(r, 800));
    } else {
  ...
  return (
    <div style={styles.game}>
      ...
      <div className="game-subtitle" style={styles.subTitle}>اضغط على حروف الكسر لتذويب الجليد</div>
