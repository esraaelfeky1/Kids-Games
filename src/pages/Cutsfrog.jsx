...
export default function MeltIceGame() {
  ...
  const [snowmanPos, setSnowmanPos] = useState({ x: 90, y: 440 });
  const [isMoving, setIsMoving] = useState(false);

  const letters = ["ح", "أ", "ش", "ز", "ث", "و", "د", "ق", "ذ", "ك", "أ", "س", "و", "ر", "د"];
  ...
  return (
    <div style={styles.game}>
      ...
      <div className="game-subtitle" style={styles.subTitle}>اضغط على حروف القطع لتذويب الجليد</div>

      {/* المربعات نزلت لتحت مسافة أوسع شوية ومظبوطة */}
      ...
      {win && (
        <div style={styles.winStyle}>
          <h1 style={{fontSize: "35px"}}>🎉 أحسنت يا بطل! 🎉</h1>
          <p style={{fontSize: "20px"}}>لقد أتممت المهمة بنجاح.</p>
        ...
        * { box-sizing: border-box; }
        ...
        @media (max-width: 767px) {
          ...
          .game-grid {
            ...
            gap: 6px !important;
            margin-top: 18px !important; /* مسافة أنزل شوية للمربعات على الموبايل */
          }
          .game-tile {
...
const styles = {
  ...
  box: { background: "white", padding: "6px 12px", borderRadius: 10, fontWeight: "bold", fontSize: "16px", width: "fit-content" },
  mainTitle: { background: "#29b6f6", color: "white", padding: "4px 18px", borderRadius: 20, fontSize: "26px", fontWeight: "bold" },
  subTitle: { 
  ...
  grid: { 
    display: "grid", 
    gridTemplateColumns: "repeat(5, 68px)", 
    gap: "8px", 
    marginTop: "20px", // نزلنا المربعات مسافة إضافية واضحة ومظبوطة تحت الفقرة
  },
  tile: { width: 62, height: 62, borderRadius: 15, background: "#dff4ff", position: "relative", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", border: "3px solid white", },
  letter: { fontSize: "30px", fontWeight: "bold", color: "#5c3b1e", zIndex: 3 },
  iceLayer: { position: "absolute", inset: 0, background: "rgba(180, 230, 255, 0.7)", borderRadius: 15, zIndex: 2, width: "100%", height: "100%" },
  snowman: { position: "absolute", width: "120px", zIndex: 10, left: "25px", bottom: "30px", transition: "all 0.8s ease" },
  bottomButtons: { position: "absolute", bottom: "20px", display: "flex", gap: "15px", zIndex: 10 },
